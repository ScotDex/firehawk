...
func handleStatusCommand(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if err != nil {
		...
		sendErrorResponse(s, i, "Server possibly offline, or you are on a VPN...")
		return
	}
	...
	embed := &discordgo.MessageEmbed{
		Author:    &discordgo.MessageEmbedAuthor{Name: "Tranquility", URL: "https://esi.evetech.net/ui/", IconURL: "https://web.ccpgamescdn.com/forums/img/eve_favicon.ico"},
		Title:     "Server Status - Online",
		Color:     0x00ff00,
		Thumbnail: &discordgo.MessageEmbedThumbnail{URL: fmt.Sprintf("https://images.evetech.net/corporations/%d/logo?size=128", getRandomCachedCorpLogo())},
...
func handleLookupCommand(s *discordgo.Session, i *discordgo.InteractionCreate) {
	s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{Type: discordgo.InteractionResponseDeferredChannelMessageWithSource})

	options := parseOptions(i)
	charName := options["character_name"].StringValue()

	charID, err := esiClient.GetCharacterID(charName)
	if err != nil {
		sendErrorResponse(s, i, fmt.Sprintf("❌ Could not find a character named `%s`.", charName))
		return
	}
	...
	embed := &discordgo.MessageEmbed{
		...
		URL:         finalURL,
		Color:       0x42b6f5,
		Thumbnail:   &discordgo.MessageEmbedThumbnail{URL: fmt.Sprintf("https://images.evetech.net/characters/%d/portrait", charID)},
		Timestamp:   time.Now().Format(time.RFC3339),
	}
	sendEmbedResponse(s, i, embed)
}
...
func handleSubscribeCommand(s *discordgo.Session, i *discordgo.InteractionCreate) {
	options := parseOptions(i)
	topic := options["topic"].StringValue()
...
func handleUnsubscribeCommand(s *discordgo.Session, i *discordgo.InteractionCreate) {
	options := parseOptions(i)
	topicToRemove := options["topic"].StringValue()
	var channelID string
	if channelOption, ok := options["channel"]; ok {
		channelID = channelOption.ChannelValue(s).ID
	} else {
...
// parseOptions converts interaction options into an easy-to-use map.
...
// sendErrorResponse sends a simple error message back to the user.
...
// sendEmbedResponse sends a rich embed back to the user.
func sendEmbedResponse(s *discordgo.Session, i *discordgo.InteractionCreate, embed *discordgo.MessageEmbed) {
	...
	if err != nil {
		log.Printf("Failed to send embed response: %v", err)
	}
}
...
func getRandomCachedCorpLogo() int {
	...
	if len(esiClient.corporationNames) == 0 {
		return 1000001 // default to CONCORD
	}
